...
- Without subscriber_id: Traditional fire-and-forget (events lost if not listening)
...
class PubSub:
    ...
    @classmethod
    async def create(cls, mongo_client=None, *args, **kwargs):
        """Create and return a PubSub object asynchronously"""
        pubsub = cls(mongo_client, *args, **kwargs)
        await pubsub._init()
        return pubsub
    ...
    def __init__(self, mongo_client=None, host=None, db_number=None,
        ...
        # {sub_id: {'sub': Subscription, 'redis_sub': PubSub, 'subscriber_id': str|None, ...}}
    ...
    async def subscribe(self, channel: str, user: str,
        ...
        async with self._lock:
            ...
            self._subscriptions[sub_id] = {
                ...
                'catchup_done': False if subscriber_id else True,
            }
    ...
    async def _setup_durable_subscription(self, sub_id: int, subscriber_id: str,
        ...
        if existing:
            ...
            async with self._lock:
                ...
                self._subscriptions[sub_id]['last_acked_id'] = existing['last_event_id']
            logger.info(
    ...
    async def publish_cloudevent(self, channel: str, data: Any,
        ...
        Events are:
        1. Stored in MongoDB eventhistory (for durable subscribers and /events API)
